...
gatingset_file <- '/loc/no-backup/mike/HVTN080output/gs1.tar'
# gatingset_file <- '/loc/no-backup/mike/HVTN080output/gs2.tar'
...
unarchive_path <- '/loc/no-backup/ramey'

# Restore the GS from the tar file specified in 'gatingset_file'
gs_manual <- unarchive(gatingset_file, path = unarchive_path)
...
lapply(list.files("~/rglab/HIMCLyoplate/Gottardo/pipeline/R", full = TRUE), source)
gating(gating_template, gs_negctrl, batch = TRUE, nslaves = 8)
gating(gating_template, gs_ENV, batch = TRUE, nslaves = 8)
gating(gating_template, gs_GAG, batch = TRUE, nslaves = 8)
...
save(population_stats, file = "data/HVTN080-results.RData")
